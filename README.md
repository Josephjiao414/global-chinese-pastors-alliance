...
这是“全球华人教牧同工联盟”的官方中文主页首版，使用 Next.js、TypeScript、Tailwind CSS 与 App Router 构建。首版不使用数据库，网站文案和活动信息保存在本地数据文件中，并已预备简体中文与繁体中文两个版本。
...
- `content/site.zh-Hant.ts`：繁体中文网站文案
- `content/events.zh-Hant.ts`：繁体中文聚会活动数据

新增活动时，需要在简体与繁体两个活动数据文件中同步追加活动对象。会议号码、会议密码、报名链接等字段为空时，页面会显示“确认后公布”或对应提示。
